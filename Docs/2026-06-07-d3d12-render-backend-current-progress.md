...
- Same-scene parity captures were recorded outside source control: `build\d3d12_parity\default_D3D12.png` and `build\d3d12_parity\default_Vulkan.png`; both captures were `1280x720` with sampled non-black ratio `100.0000%`. A 4-pixel-step comparison over 57,600 sampled pixels reported average RGB L1 delta `1.49` and max RGB L1 delta `132`.
