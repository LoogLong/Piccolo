...
namespace Piccolo
{
    ...
    void MainCameraPass::setupRenderPass()
    {
        ...
        initColorAttachment(attachments[_main_camera_pass_gbuffer_c],
                            m_framebuffer.attachments[_main_camera_pass_gbuffer_c].format,
                            RHI_ATTACHMENT_LOAD_OP_CLEAR,
                            RHI_ATTACHMENT_STORE_OP_DONT_CARE,
                            RHI_IMAGE_LAYOUT_UNDEFINED);
        ...
        if (m_rhi->createRenderPass(&create_info, m_framebuffer.render_pass) != RHI_SUCCESS)
        {
    ...
    void MainCameraPass::setupSwapchainFramebuffers()
    {
        ...
        for (size_t i = 0; i < m_rhi->getSwapchainInfo().imageViews.size(); i++)
        {
            ...
            if (m_rhi->createFramebuffer(&framebuffer_create_info, m_swapchain_framebuffers[i]) != RHI_SUCCESS)
            {
    ...
    void MainCameraPass::setupPathTracingCompositeSwapchainFramebuffers()
    {
        ...
        for (size_t i = 0; i < m_rhi->getSwapchainInfo().imageViews.size(); i++)
        {
            ...
            if (m_rhi->createFramebuffer(&framebuffer_create_info, m_path_tracing_composite_swapchain_framebuffers[i]) !=
                RHI_SUCCESS)
            {
